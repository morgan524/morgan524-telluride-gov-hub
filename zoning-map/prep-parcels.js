...
const hcRings = [];
for (const f of zon.features) if (String((f.properties || {}).ZONING || '') === 'HIGH COUNTRY AREA') hcRings.push(...ringsOf(f.geometry));
...
for (const f of parcelData.features) {
  ...
  const hc = c ? hcRings.some(r => ptInRing(c[0], c[1], r)) : false;
  if (hc) { f.properties = f.properties || {}; f.properties.high_country = 'True'; tagged++; }
}
const rounded = roundCoords(parcelData);
fs.writeFileSync(path.join(outDir, 'parcel.json'), JSON.stringify(rounded));
console.log('tagged high_country:', tagged);
