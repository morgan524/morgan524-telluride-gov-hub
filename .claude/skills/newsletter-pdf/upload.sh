...
# newsletter/<topic>/ and print their public URLs. Fast: a throwaway worktree
# off origin/main + a direct push to main (no PR), and it does NOT wait on the
# GitHub Pages build — the caller verifies links (that wait was the slow part).
#
# IMPORTANT: use a FULL checkout, NOT `git worktree add --no-checkout`. With
# --no-checkout the index is empty, so committing only the staged new file
# records a tree containing JUST that file — silently DELETING the entire rest
# of the repo. (Learned the hard way: it wiped main once; recovered via revert.)
#
# Usage:
...
git -C "$REPO" worktree add -q -b "$BR" "$WT" origin/main   # FULL checkout — never --no-checkout (see header)
