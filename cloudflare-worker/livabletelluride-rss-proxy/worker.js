...
function articleEmailHtml(doc, reviewUrl) {
  ...
  <div style="background:#21443c;color:#fff;padding:18px 22px;border-radius:8px 8px 0 0">
    <div style="font-size:12px;letter-spacing:.1em;text-transform:uppercase;color:#e7b24a">${doc.kind === "recap" ? "Meeting recap &middot; for your approval" : "Editorial &middot; Draft for review"}</div>
  ...
  if (env.MAIL_RELAY_URL) {
    try {
      await fetch(env.MAIL_RELAY_URL, {
        ...
        body: JSON.stringify({ kind: "editorial", to: MOD_RECIPIENT, subject: (doc.kind === "recap" ? "📝 Meeting recap for review: " : "📝 Draft for review: ") + (doc.title || id), html: articleEmailHtml(doc, reviewUrl), secret: env.MAIL_RELAY_SECRET || "" }),
      });
      emailed = true;
    } catch (e) { /* best-effort */ }
  }
