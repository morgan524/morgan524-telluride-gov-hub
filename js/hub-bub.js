...
(function() {
  ...
  function hbUpdateAuthUI(user) {
    // Anonymous sessions exist ONLY for Question-of-the-Day voting (see
    // hbDQVote). For the forum UI they count as signed-out: hbUser stays
    // null so the composer/react gates still route to the signup modal —
    // and Firestore's posts rules would deny an anonymous write anyway.
  ...
  // QUESTION OF THE DAY
  // ═══════════════════════════════
  // One Rick-voiced question per MT day, generated by content-refresh Task 24
  // into data/daily-questions.json. Votes are uids in choiceN_voters arrays on
  // Firestore daily_questions/{date} (rules mirror the reactions pattern:
  // anonymous sign-in is enough to vote; one-vote-per-user + vote switching
  // are client-managed, consistent with the rest of the site).
  var hbDQ = null;              // the question currently rendered
  var hbDQCounts = null;        // last-fetched per-choice vote counts

  function hbDQTodayMT() {
    return new Date().toLocaleDateString('en-CA', { timeZone: 'America/Denver' });
  }
  function hbDQChosen(date) {
    var v = localStorage.getItem('hb_dq_' + date);
    return v === null ? null : parseInt(v, 10);
  }

  function hbInitDailyQuestion() {
    var card = document.getElementById('hbDailyQ');
    if (!card) return;
    // Same origin fallback as LTData: file:// previews read production data.
    var origin = (/^https?:$/.test(location.protocol)) ? '' : 'https://livabletelluride.org';
    fetch(origin + '/data/daily-questions.json?v=' + Math.floor(Date.now() / 600000))
      .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(function (list) {
        var today = hbDQTodayMT();
        // Newest question that isn't future-dated and is at most 2 days old
        // (a generation hiccup shouldn't blank the card, but week-old
        // questions shouldn't masquerade as today's either).
        var q = (list || []).filter(function (x) {
          return x && x.date && x.title && x.body && Array.isArray(x.choices)
            && x.choices.length >= 2 && x.date <= today;
        }).sort(function (a, b) { return a.date < b.date ? 1 : -1; })[0] || null;
        if (q) {
          var d = q.date.split('-'), t = today.split('-');
          var ageDays = (Date.UTC(t[0], t[1] - 1, t[2]) - Date.UTC(d[0], d[1] - 1, d[2])) / 86400000;
          if (ageDays > 2) q = null;
        }
        if (!q) return;                    // no card — feed renders as before
        hbDQ = q;
        hbDQRender();
        hbDQFetchCounts();
      })
      .catch(function (e) { console.error('[Hub-Bub] daily question load failed:', e); });
  }

  function hbDQRender() {
    var card = document.getElementById('hbDailyQ');
    var q = hbDQ;
    var chosen = hbDQChosen(q.date);
    var isToday = q.date === hbDQTodayMT();
    var dayLabel = isToday ? 'Question of the Day' :
      'Question of the Day · ' + new Date(q.date + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'long', timeZone: 'America/Denver' });
    var total = 0, counts = [];
    for (var i = 0; i < q.choices.length; i++) {
      var n = (hbDQCounts && hbDQCounts[i]) || 0;
      counts.push(n); total += n;
    }
    var html =
      '<div class="hb-dq-kicker">❓ ' + hbEsc(dayLabel) + '</div>' +
      '<div class="hb-dq-title">' + hbEsc(q.title) + '</div>' +
      '<div class="hb-dq-body">' + q.body.split(/\n\n+/).map(function (p) { return '<p>' + hbEsc(p) + '</p>'; }).join('') + '</div>' +
      '<div class="hb-dq-choices">';
    for (var c = 0; c < q.choices.length; c++) {
      var picked = chosen === c;
      var pct = (chosen !== null && total > 0) ? Math.round(counts[c] * 100 / total) : 0;
      html += '<button type="button" class="hb-dq-choice' + (picked ? ' picked' : '') + '" onclick="hbDQVote(' + c + ')">' +
        (chosen !== null ? '<span class="hb-dq-bar" style="width:' + pct + '%"></span>' : '') +
        '<span class="hb-dq-choice-label">' + (picked ? '✓ ' : '') + hbEsc(q.choices[c]) + '</span>' +
        (chosen !== null ? '<span class="hb-dq-pct">' + pct + '%</span>' : '') +
        '</button>';
    }
    html += '</div><div class="hb-dq-foot">' +
      (chosen !== null ? '<span class="hb-dq-total">' + total + (total === 1 ? ' vote' : ' votes') + (chosen !== null ? ' · tap another to change' : '') + '</span>' : '<span class="hb-dq-total">Tap to vote — no account needed</span>') +
      '<span class="hb-dq-actions">' +
      (q.sourceUrl ? '<a href="' + hbEsc(q.sourceUrl) + '" ' + (/^https?:/.test(q.sourceUrl) ? 'target="_blank" rel="noopener"' : '') + '>Background</a> · ' : '') +
      '<a href="#" onclick="hbDQAddTake(); return false;">💬 Add your take</a></span></div>';
    card.innerHTML = html;
    card.style.display = '';
  }

  function hbDQFetchCounts() {
    if (!hbFirebaseReady || !hbDQ) return;
    db.collection('daily_questions').doc(hbDQ.date).get().then(function (snap) {
      var data = snap.exists ? snap.data() : {};
      hbDQCounts = [];
      for (var i = 0; i < hbDQ.choices.length; i++) {
        var arr = data['choice' + i + '_voters'];
        hbDQCounts.push(Array.isArray(arr) ? arr.length : 0);
      }
      hbDQRender();
    }).catch(function (e) { console.error('[Hub-Bub] daily question counts failed:', e); });
  }

  window.hbDQVote = function (idx) {
    if (!hbFirebaseReady || !hbDQ) return;
    var q = hbDQ;
    var prev = hbDQChosen(q.date);
    if (prev === idx) return;              // tapping your own choice again: no-op
    // Any Firebase session can vote; visitors without an account get an
    // anonymous one (same pattern as events.html / legal-notices.html).
    var ensure = auth.currentUser ? Promise.resolve(auth.currentUser)
      : auth.signInAnonymously().then(function (c) { return c.user; });
    ensure.then(function (user) {
      var upd = {};
      upd['choice' + idx + '_voters'] = firebase.firestore.FieldValue.arrayUnion(user.uid);
      if (prev !== null && prev !== idx && prev >= 0 && prev < q.choices.length) {
        upd['choice' + prev + '_voters'] = firebase.firestore.FieldValue.arrayRemove(user.uid);
      }
      return db.collection('daily_questions').doc(q.date).set(upd, { merge: true });
    }).then(function () {
      localStorage.setItem('hb_dq_' + q.date, String(idx));
      hbDQFetchCounts();                   // re-render with fresh counts
    }).catch(function (e) {
      console.error('[Hub-Bub] vote failed:', e);
      alert('Vote didn’t go through — please try again.');
    });
  };

  window.hbDQAddTake = function () {
    if (!hbDQ) return;
    hbExpandCompose();                     // opens signup modal if signed out
    var titleEl = document.getElementById('hbComposeTitle');
    if (titleEl && !titleEl.value) {
      titleEl.value = 'Re: ' + hbDQ.title;
      if (typeof hbUpdatePostBtn === 'function') hbUpdatePostBtn();
    }
    var anchor = document.getElementById('hb-composer-anchor');
